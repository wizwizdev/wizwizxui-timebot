    ...
    if($orders->num_rows>0){
        while ($order = $orders->fetch_assoc()){
            ...
            if($expiryTime != null && $total != null){
                ...
                if($send != ""){  
                    $msg = "💡 کاربر گرامی، 
        از سرویس اشتراک $remark تنها (۱ $send) باقی مانده است. میتواند از قسمت خرید های من سرویس فعلی خود را تمدید کنید یا سرویس جدید خریداری کنید.درصورت تمدید نشدن سرویس بعداز 10 روز از لیست سرویس های شما حذف خواهد شد.";
                    sendMessage( $msg, null, null, $from_id);
                    $newTime = $time + 86400 * 10;
    ...
    if($orders->num_rows>0){
        while ($order = $orders->fetch_assoc()){
            ...
            if($send){  
        		...
        		if(!is_null($res)){
                    $msg = "💡 کاربر گرامی،
    اشتراک سرویس $remark منقضی شد و از اتمام تاریخ آن 10 روز گذشته است و از لیست سفارش ها حذف گردید. لطفا از فروشگاه, سرویس جدید خریداری کنید.";
                    sendMessage( $msg, null, null, $from_id);
